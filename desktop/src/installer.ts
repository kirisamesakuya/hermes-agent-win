import { BrowserWindow, ipcMain } from "electron";
import { spawn } from "node:child_process";
import { InstallEvent } from "./types.js";
import { LogBuffer } from "./logBuffer.js";

const INSTALL_COMMAND =
  "irm https://raw.githubusercontent.com/NousResearch/hermes-agent/main/scripts/install.ps1 | iex";

let installing = false;

function broadcast(event: InstallEvent): void {
  for (const win of BrowserWindow.getAllWindows()) {
    win.webContents.send("hermes-install-event", event);
  }
}

export function registerInstallerIpc(logs: LogBuffer): void {
  ipcMain.handle("hermes:install", async () => {
    if (installing) return { started: false };
    installing = true;
    broadcast({ type: "start", message: "Starting Hermes Windows installer..." });
    logs.append("\n[installer] Starting Hermes Windows installer\n");

    const child = spawn(
      "powershell.exe",
      ["-NoProfile", "-ExecutionPolicy", "Bypass", "-Command", INSTALL_COMMAND],
      { windowsHide: true }
    );

    child.stdout.on("data", (chunk: Buffer) => {
      const message = chunk.toString();
      logs.append(message);
      broadcast({ type: "stdout", message });
    });

    child.stderr.on("data", (chunk: Buffer) => {
      const message = chunk.toString();
      logs.append(message);
      broadcast({ type: "stderr", message });
    });

    child.on("error", (error) => {
      installing = false;
      logs.append(`[installer] ${error.message}\n`);
      broadcast({ type: "error", message: error.message });
    });

    child.on("exit", (code) => {
      installing = false;
      logs.append(`[installer] exited with ${code}\n`);
      broadcast({ type: "exit", message: `Installer exited with ${code}`, code });
    });

    return { started: true };
  });
}
